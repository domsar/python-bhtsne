...
def tsne(data, data_name, dimensions=2, perplexity=30.0, theta=0.5, rand_seed=-1, seed_positions=np.array([])):
    tsne = BHTSNE()
    ...
    Y = tsne.run(data, data_name, data.shape[0], data.shape[1], dimensions, perplexity, theta, rand_seed, seed_positions, skip_random_init)
    return Y
